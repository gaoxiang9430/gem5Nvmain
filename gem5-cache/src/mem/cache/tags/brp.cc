...
#define NVM_r_e 4     //define the energy consumption of NVM/DRAM operation
#define NVM_w_e 33
#define DRAM_r_e 15
#define DRAM_w_e 5

#define NVM_r_t 24	   //define the latency of NVM/DRAM operation
#define NVM_w_t 70
#define DRAM_r_t 9
#define DRAM_w_t 10

#define SAMPLE_SET 32
    ...
    : BaseSetAssoc(p)
{
    ...
    skip_set = number_set/SAMPLE_SET;
    
    hit = 0;
    core = new int*[number_set];
    accessPCMTimes = new int*[number_set];
    accessDRAMTimes = new int*[number_set];
    ...
    for(int i=0;i<number_set;i++)
    {
    	...
    	accessPCMTimes[i] = new int[NUM_CORE];
    	accessDRAMTimes[i] = new int[NUM_CORE];
    	for(int j=0;j<NUM_CORE;j++)
    	{
    		accessPCMTimes[i][j]=0;
    		accessDRAMTimes[i][j]=0;
    	}
    }
	for(int i=0;i<NUM_CORE;i++)
	{
		core_sampling[i] = (assoc+1)/NUM_CORE;
		access[i] = 0;
		hits[i] = 0;
	}
...
BRP::record_brp(Addr addr,MemCmd cmd,int portId)
{
	...
	/*if(addr >= PhysicalMemory::watershed) //the access address is DRAM
	{
		accessDRAMTimes[set][portId]++;
	}
	else
		accessPCMTimes[set][portId]++;*/
	...
	for(unsigned i=0;i<assoc;i++)
	{
		if(Temp_sets[set].blks[i]->tag == tag)  //hit the emulator cache line when access cache lines
		{
			...
			{
				if(isPcm)
					Temp_sets[set].blks[i]->asid_dirty += NVM_w_t;
				else
					Temp_sets[set].blks[i]->asid_dirty += DRAM_w_t;
			}
				//Temp_sets[set].blks[i]->asid_dirty ++;
			else
			{
				if(isPcm)
					Temp_sets[set].blks[i]->asid += NVM_r_t;
				else
					Temp_sets[set].blks[i]->asid += DRAM_r_t;
			}
...
BRP::accessBlock(Addr addr, bool is_secure, Cycles &lat, int master_id,MemCmd cmd,int portId)
{
    ...
    if (blk != NULL) {
		...
		if(!warmedUp)
	        ...
	        if(blk->isPcm)
		        sets[blk->set].moveNStep(blk,core_sampling[portId]/2+1);
    		else
    	    	sets[blk->set].moveNStep(blk,core_sampling[portId]/4+1);
    	}    
...
BRP::setRecord()                  //cpu will call this function every 50M instructions
{
	...
	{
		accessPCM[i] = 0;
		accessDRAM[i] = 0; //initialize the accessPCM and accessDRAM
	}
	for(int i=0;i<number_set;i++)
	{
		for(int k=0;k<NUM_CORE;k++)
		{
			if(!(i&(skip_set-1)))
			{
				core_sampling[k] += core[i][k];
				accessPCM[k] = accessPCMTimes[i][k];
  				accessDRAM[k] = accessDRAMTimes[i][k];
  				accessPCMTimes[i][k] = 0;
  				accessDRAMTimes[i][k] = 0;
			}
		}
	}
	for(int k=0;k<NUM_CORE;k++)
	{
		core_sampling[k] = core_sampling[k]/SAMPLE_SET;
		DPRINTF(HAP, "core %d : PCM : DRAM = %d %d\n",k,accessPCM[k],accessDRAM[k]);
	}
	DPRINTF(HAP, "set partition : %d %d %d %d\n",core_sampling[0],core_sampling[1],core_sampling[2],core_sampling[3]);
}
//end
...
//xiang
	...
	else
	{
		...
		int dram_offset = 2;
		int pcm_offset = 4;
		/*if(accessDRAM[portId]!=0)
			pcm_offset =(int)(pcm_offset*(double)(accessPCM[portId])/accessDRAM[portId]); 
		*/    
		if(blk->isPcm)
			position = (core_sampling[portId]+pcm_offset > assoc-1)?assoc-1:core_sampling[portId]+pcm_offset;
		else
			position = (core_sampling[portId]-dram_offset>0)?core_sampling[portId]-dram_offset:0;
