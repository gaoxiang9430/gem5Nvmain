...
BRP::record_brp(Addr addr,MemCmd cmd)
{
	...
	if(access_Time==120000)
	{
		...
		int DRAM_save_overhead[assoc];
		int NVM_save_overhead[assoc];
		...
		for(int i=0;i<number_set;i++)
		{
			...
			for(int j=0;j<assoc;j++)
			{
				...
				DRAM_save_overhead[j] = DRAM_sets[i].blks[j]->asid * DRAM_r_e + DRAM_sets[i].blks[j]->asid_dirty * (DRAM_r_e + DRAM_w_e);
			...
			{	
				...
				//calculator the total over head if the cache was ejected
				NVM_save_overhead[j] = NVM_sets[i].blks[j]->asid * NVM_r_e + NVM_sets[i].blks[j]->asid_dirty * (NVM_r_e + NVM_w_e);
			...
			{
				DRAM_save_overhead[k] += DRAM_save_overhead[k-1];
				sprintf(tmp2,"%d ",DRAM_save_overhead[k]);
				strcat(tmp,tmp2);
			}
			DPRINTF(HAP, "DRAM set %d : %s\n",i,tmp);
			...
			for(int k=1;k<assoc;k++)
			{				
				NVM_save_overhead[k] += NVM_save_overhead[k-1];	
				sprintf(tmp2,"%d ",NVM_save_overhead[k]);
				strcat(tmp,tmp2);
			}
			DPRINTF(HAP, "NVM set %d : %s\n",i,tmp);
			
			int result = find_best(DRAM_save_overhead, NVM_save_overhead);
			DPRINTF(HAP, "set %d : %d\n",i,result);
			//DPRINTF(HAP, "DRAM set %d : %s\n",i,tmp);
			...
			find_best(DRAM_save_overhead, NVM_save_overhead);
			//DPRINTF(HAP, "NVM set %d : %s\n",i,tmp);*/
		}
        access_Time = 0;
	}
...
//find the best partition strategy, result is the NVM_ways
int 
BRP::find_best(int *DRAM_save_overhead,int *NVM_save_overhead)
{
	int result = -1;
	int current_overhead = 0;
	for(int i=0;i<assoc;i++)
	{
		int overhead = DRAM_save_overhead[i] + NVM_save_overhead[assoc-1-i];
