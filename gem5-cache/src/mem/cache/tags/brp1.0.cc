...
int 
BRP::subBest(int level, int tempBest, int remain)
{
	int res,ret=0;
	...
	res = subBest(level+1,tempBest,remain);
	ret = res+ret;
	if(res)
	{
		res = 0;
	...
	for(int i=0;i<remain;i++)
	{
		if(remain-i-1>=0)
		{
			res = subBest(level+1,tempBest+save_overhead[level][i],remain-i-1);
			ret = res+ret;
			if(res)
			{
				res = 0;
...
BRP::find_best()
{
	...
	for(int i=0;i<NUM_CORE;i++)
	{
		...
		for(int j=0;j<assoc;j++)
		{
			sprintf(tmp2,"%d ",save_overhead[i][j]);
			strcat(tmp,tmp2);
		}	
		DPRINTF(HAP, "core %d : %s\n",i,tmp);
	}
	subBest(0,0,assoc);  
	DPRINTF(HAP, "core %d %d %d %d\n",core[0],core[1],core[2],core[3]);
}
...
BRP::setRecord()                  //cpu will call this function every 50M instructions
{
	...
	for(int i=0;i<NUM_CORE;i++)
	{
		DPRINTF(HAP, "core %d: access %d, hits %d, hit rate:%f\n", i,access[i],hits[i],(double)hits[i]/access[i]);
		if((double)hits[i]/access[i] < 0.125)  //determain whether this benchmark is stream or not
			isStream[i] = 1;
		else isStream[i] = 0;
	...
	for(int i=0;i<number_set;i++)
	{
		...
		find_best();  //find the best partition strategy
	}
