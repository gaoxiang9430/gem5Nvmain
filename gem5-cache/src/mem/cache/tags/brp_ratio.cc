...
BRP::record_brp(Addr addr,MemCmd cmd,int portId)
{
	...
	for(unsigned i=0;i<assoc;i++)
	{
		if(Temp_sets[set].blks[i]->tag == tag)  //hit the emulator cache line when access cache lines
		{
			...
			Temp_sets[set].blks[0]->asid_dirty++;
...
BRP::setRecord()                  //cpu will call this function every 50M instructions
{
	...
	for(int i=0;i<number_set;i++)
	{
		for(int k=0;k<NUM_CORE;k++)
		{
			...
			double set_ratio = 0;
			if(Temp_sets[i].blks[0]->asid > 0)
				set_ratio = (double)Temp_sets[i].blks[0]->asid_dirty/(double)Temp_sets[i].blks[0]->asid;
			DPRINTF(HAP, "raw data core %d set %d access: %d hit: %d ratio: %lf\n",k,i,Temp_sets[i].blks[0]->asid,Temp_sets[i].blks[0]->asid_dirty,set_ratio);
			Temp_sets[i].blks[0]->asid = 0;
			Temp_sets[i].blks[0]->asid_dirty = 0;
		}
